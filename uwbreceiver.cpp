...
void UwbReceiver::waitUwbThreadsEnd(){
...
void *UwbReceiver::receivingLoop(void *arg)
{
  clock_t t;
   initialize();
                ...
                if (status_reg & SYS_STATUS_RXFCG)
                {
                    ...
                    if (memcmp(rx_buffer, rx_final_msg, ALL_MSG_COMMON_LEN) == 0)
                    {
                        ...
                        if (!report_average) {
                            ...
                            printf("DIST: %3.2f m, dt: %3.2f s\n", distance,((float)t)/CLOCKS_PER_SEC);
                        }
                        else {
                            sample_buffer[next_sample_pos++] = distance;
                            if (next_sample_pos >= max_sample_count) {
                                next_sample_pos = 0;
                            }
                            total_sample_count++;
                            if (total_sample_count > max_sample_count) {
                                total_sample_count = max_sample_count;
                            }
                            if (kbhit()) {
                                char tmp;
                                // Clear the buffer
                                if (read(0, &tmp, sizeof(tmp)) < 1) {
                                ...
                                float avg = 0.0f;
                                float min = sample_buffer[0], max = sample_buffer[0];
                                for (int i = 0; i < total_sample_count; ++i) {
                                    avg += sample_buffer[i] / total_sample_count;
                                ...
                                printf("Average is: %3.3f, min: %3.3f, max: %3.3f\r\n", avg, min, max);
                            }
                        }
                    t = clock();
usleep(980000);
...
void UwbReceiver::startReceiveing()
{
